...
$promotion = getProductPromotion($product_id);

if ($promotion) {
    $is_flash_sale = true;
    // ISO 8601 kèm offset để JS đếm ngược đúng múi giờ
    $flash_sale_end = date('c', strtotime($promotion['end_date']));
    // Hỗ trợ cả giảm theo % lẫn theo số tiền cố định
    $discount_percent = getPromotionDiscountPercent((float)$product['price'], $promotion);
    $product['sale_price'] = calculateSalePrice((float)$product['price'], $promotion['discount_percent'] ?? 0, $promotion);
}
