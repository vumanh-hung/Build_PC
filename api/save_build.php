...
if (!isset($_SESSION['user']['user_id'])) {
    echo json_encode(['status'=>'error','message'=>'Vui lòng đăng nhập để lưu cấu hình']);
    exit;
}
...
if (empty($data['items'])) {
    echo json_encode(['status'=>'error','message'=>'Không có linh kiện nào']);
    exit;
}

$total_price = array_sum(array_column($data['items'],'price'));
...
try {
    ...
    $pdo->commit();
    echo json_encode(['status'=>'success','message'=>'Lưu cấu hình thành công!','build_id'=>$build_id]);
} catch (Exception $e) {
    $pdo->rollBack();
